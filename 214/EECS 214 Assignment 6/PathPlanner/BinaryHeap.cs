...
namespace PathPlanner
{
    ...
    public class BinaryHeap
    {
        ...
        public Node ExtractMin()
        {
            ...
            Node n = data[0];
            data[0] = data[count-1];
            data[0].QueuePosition = 0;
            priorities[0] = priorities[count-1];
            data[count-1] = null;
            priorities[count-1] = 0;
            count--;
            MoveDown(0);
            return n;
        }
        ...
        void MoveUp(int position){
            while (priorities[position] < priorities[Parent(position)]) {
        ...
        void MoveDown(int position) {
            if (LeftChild(position) >= data.Length)
                return;
            else if (RightChild(position) >= data.Length) {
                if (data[LeftChild(position)]!=null && priorities[LeftChild(position)] < priorities[position])
                {
                    Swap(position, LeftChild(position));
                }
                return;
            }
            if (data[LeftChild(position)] == null && data[RightChild(position)] == null)
            {
                return;
            }
            else if (data[LeftChild(position)] == null && data[RightChild(position)] != null) {
                if (priorities[RightChild(position)] < priorities[position]) {
            ...
            {
                if (priorities[LeftChild(position)] < priorities[position])
                {
                    Swap(position, LeftChild(position));
                }
                return;
            }
            if (priorities[position] > priorities[LeftChild(position)] || priorities[position] > priorities[RightChild(position)]) {
                if (priorities[RightChild(position)] < priorities[LeftChild(position)])
                {
                    Swap(position, RightChild(position));
                    MoveDown(RightChild(position));
                }
                else
                {
                    Swap(position, LeftChild(position));
                    MoveDown(LeftChild(position));
                }  
            }
        }
